 ...
 * 글로벌 chrome 라우터.
 *
 * AppShell 라우트  → `/club/[id]/*` (단, /club/{home,create,join,login,onboard} 제외)
 * SaaSShell 라우트 → `/clubs`, `/club/create`, `/clubs/*`
 * 그 외             → MarketingShell (홈, /blog, /pricing, /login, /manual 등)
 *
...
export function MainShell({ children }: { children: React.ReactNode }) {
  ...
  const isClubAppShell =
    /^\/club\/[^/]+(\/|$)/.test(pathname) &&
    !/^\/club\/(home|create|join|login|onboard)(\/|$)/.test(pathname)
